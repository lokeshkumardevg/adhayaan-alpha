...
const SignInPages = () => {
  ...
  const [courseFee, setCourseFee] = useState(
    localStorage.getItem("courseFees2") || ""
  );
  const [courseList, setCourseList] = useState([]);
  const [courseName, setCourseName] = useState(
    localStorage.getItem("courseName2") || ""
  );
  const [courseCategory, setCourseCategory] = useState(
    localStorage.getItem("courseCategory2") || ""
  );
  ...
  const handleInputChange = (e) => {
    ...
    if (value !== "") {
      setAndStoreInLocalStorage("courseName2", value);
    }
  };
  ...
  const handleItemClick = (value) => {
    ...
    if (selectedCourse) {
      ...
      if (selectedCourseDetails) {
        ...
        localStorage.setItem("courseStream2", selectedCourseDetails.stream_name);
        localStorage.setItem("course_id2", selectedCourseDetails.id);
        localStorage.setItem(
          "courseFee2",
  ...
  const handleNextClick = () => {
    ...
    setAndStoreInLocalStorage("courseName2", courseName);
    setAndStoreInLocalStorage("courseCategory2", courseCategory);
    setAndStoreInLocalStorage("courseStream2", courseStream);
    setAndStoreInLocalStorage("selectedMode2", selectedMode);
    setAndStoreInLocalStorage("courseFees2", courseFee);
    // Navigate to the next page
    navigate("/another-confirmation");
  };
